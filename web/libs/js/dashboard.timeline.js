$(document).ready(function(){
        ...
        return new Promise((resolve,reject) => {
            ...
            $.getJSON(`${getApiPrefix(customVideoSet ? customVideoSet : searchQuery ? `videosByEventTag` : `videos`)}${monitorId ? `/${monitorId}` : ''}?${requestQueries.concat([limit ? `limit=${limit}` : `noLimit=1`]).join('&')}`,function(data){
                ...
                function completeRequest(eventData){
                    ...
                    // shinobiServer.loadEventsData(theEvents)
                    // shinobiServer.loadVideosData(newVideos)
                    shinobiServer.applyPeerConnectKeyToRows(newVideos)
                    if(callback)callback({videos: newVideos});
                    resolve({videos: newVideos})
                }
                if(noEvents){
                    completeRequest([])
                }else{
    ...
    async function getVideosInGaps(gaps,monitorIds){
        ...
        async function loopOnGaps(monitorId){
            for (let i = 0; i < gaps.length; i++) {
                ...
                executeEventHandlers('timelineGetVideosByMonitor', [monitorId, videosFound])
            }
        }
    ...
    async function getVideosByTimeStripRange(addOrOverWrite){
        ...
        if(timeStripSelectedMonitors.length > 0){
            ...
            if(gaps.length > 0){
                ...
                executeEventHandlers('timelineGetVideos', [videos])
    ...
    function drawVideosToCanvas(selectedVideosByMonitorId){
        ...
        $.each(loadedMonitors,function(monitorId,monitor){
            ...
            html += `<div class="timeline-video col-${timelineGridSizing} p-0 m-0 no-video" data-peetconnectkey="${monitor.peerConnectKey}" data-mid="${monitorId}" data-ke="${monitor.ke}" style="background-color:${itemColor}">
                <div class="film"></div>
        ...
        timeStripVis.on('click', async function(properties) {
            ...
            timeStripActionWithPausePlay().then((timeChanging) => {
                if(!timeChanging){
                    ...
                    executeEventHandlers('timelineTimeChange', [clickTime, loadedMonitorsOnCanvas])
                }
            })
        });
        ...
        timeStripVis.on('rangechanged', function(properties){
            ...
            timeChangingTimeout = setTimeout(function(){
                ...
                setTimeout(() => {
                    ...
                    executeEventHandlers('timelineRangeChanged', [properties.start, properties.end, getTimestripDate(), loadedMonitorsOnCanvas])
                },500)
            },300)
        })
        setTimeout(function(){
    ...
    function timeStripPlay(forcePause){
        if(!forcePause && !isPlaying){
            ...
            timeStripVisTickMovementInterval = setInterval(function() {
                ...
                setTickDate(newTime);
                executeEventHandlers('timelineTimeChange', [newTime, loadedMonitorsOnCanvas])
    ...
    function drawFoundCamerasSubMenu(){
        ...
        if(!sideMenuListMissing){
            var treeOfMonitors = convertMonitorsToTree()
            sideMenuList.addClass('server-tree').empty()
            Tree.generate(`#side-menu-link-timeline ul`, treeOfMonitors, {
                hiddenIcon: true,
            });
        }
    ...
    function convertMonitorsToTree(peerConnectKey = shinobiServer.peerConnectKey){
        ...
        function addMonitor(monitorId, monitor, tag){
            ...
            const label = `<span class="timeline-selectMonitor" timeline-menu-action="selectMonitor" data-mid="${monitorId}" data-ke="${monitor.ke}" data-peerconnectkey="${peerConnectKey}"><span class=" dot dot-grey mr-2"></span><span>${monitor.name}</span></span>`
            tree[tag][label] = function(){
    ...
    createEventHandler('timelineTimeChange')
    createEventHandler('timelineRangeChanged')
    createEventHandler('timelineGetVideos')
    createEventHandler('timelineGetVideosByMonitor')
})
