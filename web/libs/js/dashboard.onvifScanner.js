$(document).ready(function(e){
    ...
    var peerConnectKeysList = onvifScannerWindow.find('.peerConnectKeys_list')
    ...
    function getSelectedPeerConnectKey(){
        ...
        return { peerConnectKey, shinobiServer, loadedMonitors }
    }
    ...
    function isOnvifRowAlreadyALoadedMonitor(onvifRow){
        var matches = null;
        const { loadedMonitors } = getSelectedPeerConnectKey()
    ...
    var postMonitor = function(monitorToPost){
        ...
        shinobiServer.websocket.configureMonitor(newMon).then(function(response){
            console.log(newMon.host, response)
        })
    }
    ...
    scanForm.submit(function(e){
        ...
        checkTimeout = setTimeout(function(){
            if(onvifScannerResultPane.find('.card').length === 0){
                setAsLoading(false)
                onvifScannerResultPane.append(`<div class="p-2 text-center _notfound text-white epic-text">${lang.sorryNothingWasFound}</div>`)
            }
        },5000)
        return false;
    });
    onvifScannerWindow.on('click','.copy',function(){
        const { peerConnectKey } = getSelectedPeerConnectKey()
        openMonitorEditorPage(null, peerConnectKey)
    ...
    loadLocalOptions()
    onShinobiWebSocketEvent(function (d){
        switch(d.f){
            case'onvif':
                drawProbeResult(d)
            break;
    ...
    addOnTabOpen('onvifScanner', onTabChange)
    addOnTabReopen('onvifScanner', onTabChange)
})
