...
$(document).ready(function(e){
    ...
    var peerConnectKeysList = monitorEditorWindow.find('.peerConnectKeys_list')
    ...
    var getSelectedMonitorInfo = function(){
        const groupKey = monitorEditorWindow.attr('data-ke')
        const monitorId = monitorEditorWindow.attr('data-mid')
        const peerConnectKey = peerConnectKeysList.val()
        const shinobiServer = loadedShinobiAPI[peerConnectKey]
        const authToken = shinobiServer.apiKey
        const loadedMonitors = shinobiServer.loadedMonitors
        const loadedMonitor = loadedMonitors[monitorId]
        return {
            ke: groupKey,
            mid: monitorId,
    ...
    var copyMonitorSettingsToSelected = function(monitorConfig){
        const { loadedMonitors, shinobiServer } = getSelectedMonitorInfo();
        ...
        $.each(copyMonitors,function(n,id){
            ...
            shinobiServer.websocket.configureMonitor(monitor).then(function(d){
                debugLog(d)
            })
    ...
    window.getMonitorEditFormFields = function(){
        ...
        var errorsFound = []
        if(!monitorConfig.mid)monitorConfig.mid = generateId(7);
        if(!monitorConfig.name)monitorConfig.name = `Cam_${monitorConfig.mid}`;
        response.peerConnectKey = peerConnectKeysList.val()
        ...
        if(response.peerConnectKey == ''){errorsFound.push('No Server Selected')}
        //edit details
        ...
        monitorConfig.details.input_maps = getInputMapFields()
    ...
    function drawMonitorSettingsSubMenu(){
        drawSubMenuItems('monitorSettings',pageLayouts['Monitor Settings'])
    }
    ...
    async function drawInputMapHtml(options){
        var tmp = ''
        ...
        var inputMapContainer = $(`#monSectionMap${tempID}`)
        await getHardwareAccels(function(theList, listHtml){
            inputMapContainer.find(`[map-detail="hwaccel"]`).html(listHtml)
        })
        inputMapContainer.find('[map-detail="aduration"]').change()
        return tempID;
    }
    ...
    function buildMapSelectorOptionsBasedOnAddedMaps(){
        var baseOptionSet = pageLayouts['Monitor Settings'].blocks.Input.info.find((item) => {return item.name === 'detail=primary_input'}).possible
    ...
    function getStorageLocations(){
        return new Promise((resolve) => {
            $.get(getApiPrefix('storageLocations'),function(data){
                var theList = data.list || [];
                var listHtml = theList.map(item => createOptionHtml(item)).join('')
                var fieldEl = editorForm.find(`[detail="dir"]`);
                fieldEl.html(listHtml)
                try{
                    const { peerConnectKey, mid: monitorId, loadedMonitor } = getSelectedMonitorInfo();
                    if(loadedMonitor){
                        var selectedValue = loadedMonitor.details.dir
                        fieldEl.val(selectedValue)
                    }
                }catch(err){
                    console.log(err)
                }
    ...
    function getHardwareAccels(callback){
        return new Promise((resolve) => {
            $.get(getApiPrefix('hardwareAccels'),function(data){
                var theList = data.list || [];
                var listHtml = theList.map(item => createOptionHtml(item)).join('')
                if(callback){
                    callback(theList, listHtml)
                }else{
                    var fieldEl1 = editorForm.find(`[detail="hwaccel"]`);
                    var fieldEl2 = editorForm.find(`[detail-substream-input="hwaccel"]`);
                    fieldEl1.html(listHtml)
                    fieldEl2.html(listHtml)
                    try{
                        const { peerConnectKey, mid: monitorId, loadedMonitor } = getSelectedMonitorInfo();
                        if(loadedMonitor){
                            var selectedValue1 = loadedMonitor.details.dir
                            var selectedValue2 = loadedMonitor.details.substream.input.hwaccel
                            fieldEl1.val(selectedValue1)
                            fieldEl2.val(selectedValue2)
                        }
                    }catch(err){
                        console.log(err)
                    }
    ...
    async function importIntoMonitorEditor(options){
        ...
        monitorEditorWindow.attr('data-mid',monitorConfig.mid).attr('data-ke',monitorConfig.ke).attr('data-peerconnectkey',monitorConfig.peerConnectKey)
        peerConnectKeysList.val(monitorConfig.peerConnectKey)
        ...
        if(monitorDetails.input_maps && monitorDetails.input_maps !== ''){
            ...
            if(input_maps.length > 0){
                ...
                for(v of input_maps){
                    var tempID = await drawInputMapHtml()
        ...
        await getHardwareAccels()

        copySettingsSelector.val('0').change()
        var tmp = '';
        $.each(loadedShinobiAPI[monitorConfig.peerConnectKey].loadedMonitors,function(n,monitor){
    ...
    editorForm.submit(function(e){
        ...
        if(!validation.ok){
            ...
            console.log(errorsFound.join('<br>'));
        ...
        var postFormVals = monitorConfig;
        // var postFormVals = getChangedObjectValues(alreadyExisting, monitorConfig);
        // postFormVals.ke = monitorConfig.ke;
        // postFormVals.mid = monitorConfig.mid;
        ...
        if(Object.keys(postFormVals).length > 0){
            ...
            shinobiServer.websocket.configureMonitor(postFormVals)
                .then(function(d){
                    if(d.ok === false){
                        new PNotify({
    ...
    monitorEditorWindow.find('.save_config').click(function(e){
        ...
        const { mid: monitorId, peerConnectKey } = getSelectedMonitorInfo();
        var form = editorForm.serializeObject();
        form.details = safeJsonParse(form.details)
        downloadJSON(form,'Shinobi_'+monitorId+peerConnectKey+'_config.json')
    });
    ...
    function addDetectorPlugin(name, d, peerConnectKey){
        const shinobiServer = loadedShinobiAPI[peerConnectKey]
        shinobiServer.connectedDetectorPlugins[d.plug] = {
            id: d.id,
            plug: d.plug,
            notice: d.notice,
    ...
    function removeDetectorPlugin(name, peerConnectKey){
        const shinobiServer = loadedShinobiAPI[peerConnectKey]
        delete(shinobiServer.connectedDetectorPlugins[name])
        drawPluginElements(name)
    }
    function drawPluginElements(){
        const { shinobiServer } = getSelectedMonitorInfo();
        if(Object.keys(shinobiServer.connectedDetectorPlugins).length === 0){
    ...
    function resetMonitorEditorFields(){
        const currentServer = `${peerConnectKeysList.val()}`
        ...
        monitorsList.val('')
        peerConnectKeysList.val(currentServer)
    }
    window.openMonitorEditorPage = function(monitorId, peerConnectKey = peerConnectKeysList.val()){
        const shinobiServer = loadedShinobiAPI[peerConnectKey]
        const loadedMonitors = shinobiServer.loadedMonitors
        ...
        monitorsList.val(loadedMonitors[monitorConfigToLoad.mid] ? monitorConfigToLoad.mid : '')
        peerConnectKeysList.val(peerConnectKey)
    }
    function onMonitorEdit(d, peerConnectKey){
        const shinobiServer = loadedShinobiAPI[peerConnectKey]
        const monitorId = d.mid || d.id
        const newMonitorData = d.mon
        const loadedMonitors = shinobiServer.loadedMonitors;
        var loadedMonitor = loadedMonitors[monitorId]
        newMonitorData.peerConnectKey = peerConnectKey;
        ...
        $.each(newMonitorData,function(n,v){
            shinobiServer.loadedMonitors[monitorId][n] = n === 'details' ? safeJsonParse(v) : v
        })
        if(d.new === true){
    ...
    function clearMonitorTimers(monitorId){
        try{
            const { loadedMonitors } = getSelectedMonitorInfo();
    ...
    peerConnectKeysList.change(async function(){
    ...
    //     console.log('Opened Monitor Settings')
    ...
    .on('click','.delete-monitor-in-settings-window',function(){
        var validation = getMonitorEditFormFields()
        var monitorConfig = validation.monitorConfig
        var peerConnectKey = validation.peerConnectKey
        var shinobiServer = loadedShinobiAPI[peerConnectKey]
        if(shinobiServer.loadedMonitors[monitorConfig.mid]){
            shinobiServer.deleteMonitors([monitorConfig],function(){
                openMonitorEditorPage()
                goBackOneTab()
            })
        }else{
    ...
    .on('click','.export-from-monitor-settings-window',function(){
        ...
        var peerConnectKey = validation.peerConnectKey
        monitorConfig.details = safeJsonParse(monitorConfig.details)
        downloadJSON(monitorConfig,`${monitorConfig.name}_${monitorConfig.mid}_${peerConnectKey}_${formattedTime(new Date(),true)}.json`)
    })
    .on('click','.open-monitor-settings',function(){
        ...
        var peerConnectKey = thisEl.attr('data-peerconnectkey')
        if(doNew !== 'true' && !monitorId){
            var el = thisEl.parents('[data-mid]')
            monitorId = el.attr('data-mid')
            peerConnectKey = el.attr('data-peerconnectkey')
        }
        console.log(monitorId, peerConnectKey)
        openMonitorEditorPage(doNew === 'true' ? null : monitorId, peerConnectKey)
    });
    ...
    onShinobiWebSocketEvent(function (d, peerConnectKey){
        ...
        switch(d.f){
            ...
            case'monitor_edit':
                setMonitorAllCountOnUI()
                onMonitorEdit(d, peerConnectKey)
            break;
            case'detector_plugged':
                var serverIsSelected = peerConnectKeysList.val() === peerConnectKey;
                addDetectorPlugin(d.plug, d, peerConnectKey)
                if(monitorEditorSelectedMonitor && serverIsSelected)getPluginsList(monitorEditorSelectedMonitor);
            break;
            case'detector_unplugged':
                var serverIsSelected = peerConnectKeysList.val() === peerConnectKey;
                removeDetectorPlugin(d.plug, peerConnectKey)
                if(monitorEditorSelectedMonitor && serverIsSelected)getPluginsList(monitorEditorSelectedMonitor);
            break;
        }
    })
    ...
    function onTabMove(){
        ...
        var firstServer = drawPeerConnectKeysToSelector(peerConnectKeysList,true,null)
        console.log(theSelectedServer,firstServer)
        // drawMonitorListToSelector(monitorsList,false,'host')
        drawMonitorListToSelector(monitorsList.find('optgroup'),null,'host',false,firstServer)
        monitorsList.val(theSelected)
        peerConnectKeysList.val(firstServer)
        // checkToOpenSideMenu()
