$(document).ready(function(){
    ...
    window.getVideoPlayerTabId = function(video){
        return `videoPlayer-${video.peerConnectKey}-${video.mid}-${moment(video.time).format('YYYY-MM-DD-HH-mm-ss')}`
    }
    window.createVideoPlayerTab = function(video){
        var peerConnectKey = video.peerConnectKey;
        ...
        var tabLabel = `${shinobiServer.loadedMonitors[video.mid] ? shinobiServer.loadedMonitors[video.mid].name : lang['Monitor or Key does not exist.']}<br><small title="${formattedTime(video.time)}">${timeAgo(video.time)}</small>`
        ...
        if(hasRows){
            ...
            $.each(video.events,function(n,theEvent){
                ...
                var objectsFound = {}
                eventMatrixHtml += `<div class="d-block tab-videoPlayer-event-row card bg-darker mb-1 p-2" data-peerconnectkey="${peerConnectKey}" data-mid="${theEvent.mid}" data-time="${theEvent.time}">`
        ...
        var baseHtml = `<main class="page-tab tab-videoPlayer" id="tab-${newTabId}" video-id="${video.mid}${video.time}${video.type}" data-peerconnectkey="${peerConnectKey}" data-time="${video.time}" data-mid="${video.mid}" data-ke="${video.ke}" data-type="${video.type}">
                      ...
                      <div class="col-md-6">
                              ...
                              <div class="d-block pt-4">
                                  <div class="btn-group btn-group-justified">
                                        <a class="btn btn-sm btn-success download-video"><i class="fa fa-download"></i> ${lang.Download}</a>
                                        ${shinobiServer.permissionCheck('video_delete',video.mid) ? `<a class="btn btn-sm btn-danger delete-video"><i class="fa fa-trash-o"></i> ${lang.Delete}</a>` : ''}
                                        ${shinobiServer.permissionCheck('video_delete',video.mid) ? `<a class="btn btn-sm btn-${video.archive === 1 ? `success status-archived` : `default`} archive-video" title="${lang.Archive}"><i class="fa fa-${video.archive === 1 ? `lock` : `unlock-alt`}"></i> ${lang.Archive}</a>` : ''}
                                        <div class="dropdown dropup d-inline-block">
                                            ...
                                            <ul class="dropdown-menu shadow-lg">
                                                ${shinobiServer.buildDefaultVideoMenuItems(video,{
                                                    play: false
                                                })}
                                            </ul>
                                        </div>
