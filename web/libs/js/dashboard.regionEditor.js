$(document).ready(function(e){
    var theEnclosure = $('#tab-regionEditor')
    var regionEditorForm = theEnclosure.find('form')
    ...
    var regionStillImage = theEnclosure.find('.toggle-region-still-image');
    var regionEditorCanvasHolder = theEnclosure.find('.canvas_holder')
    var regionEditorMonitorsList = $('#region_editor_monitors')
    var regionEditorLiveView = $('#region_editor_live')
    var regionEditorGridOverlay = theEnclosure.find('.grid')
    var accuracyModeToggle = theEnclosure.find('[detail="detector_motion_tile_mode"]')
    var tileSizeField = theEnclosure.find('[detail="detector_tile_size"]')
    ...
    function getServerInfo(){
        ...
        const apiBaseUrl = shinobiServer.apiBaseUrl
        return { peerConnectKey, shinobiServer, apiBaseUrl }
    }
    ...
    var getRegionEditorCanvas = function(){
        return theEnclosure.find('canvas')
    }
    var getRegionEditorNameField = function(){
        return theEnclosure.find('[name="name"]')
    }
    ...
    function initLiveStream(monitorId){
        ...
        if(loadedMonitor.mode === 'stop'){
            ...
            const { apiBaseUrl, shinobiServer } = getServerInfo()
            ...
            var apiUrl = shinobiServer.buildEmbedUrl(loadedMonitor)
            if(liveElement.attr('src') !== apiUrl){
                liveElement.attr('src',apiUrl).show()
            }
            console.log(liveElement.length)
    ...
    var initCanvas = function(dontReloadStream){
        ...
        if(!regionEditorRegionsListValue){
            ...
            theEnclosure.find('.cord_name').text(cord.name || regionEditorRegionsListValue)
            ...
            theEnclosure.find('.canvas_holder canvas').remove()
    ...
    theEnclosure.find('[name]').change(function(){
        var currentRegionId = getCurrentlySelectedRegionId()
        var el = $(this)
        var val = el.val()
    ...
    theEnclosure.on('change','[point]',function(e){
        var currentRegionId = getCurrentlySelectedRegionId()
        var points = [];
    ...
    theEnclosure.find('.erase').click(function(e){
        var currentRegionId = getCurrentlySelectedRegionId()
        var newCoordinates = []
    ...
    theEnclosure.on('changed','#regions_canvas',function(e){
        drawPointsTable()
        // saveCoords()
    })
    ...
    theEnclosure.on('click','.add',function(e){
        e.stopPropagation()
        regionEditorForm.find('input').prop('disabled',false)
    ...
    peerConnectKeysList.change(function(){
        var peerConnectKey = $(this).val()
        drawMonitorListToSelector(regionEditorMonitorsList,true,null,null,peerConnectKey)
        loadEnclosure()
        var monitorId = regionEditorMonitorsList.val()
        var monitor = loadedMonitors[monitorId]
        loadRegionEditor(monitor)
        initLiveStream()
    })
    addOnTabOpen('regionEditor', function () {
        ...
        drawMonitorListToSelector(regionEditorMonitorsList,true,null,null,firstServer)
        loadEnclosure()
        var monitorId = regionEditorMonitorsList.val()
        var monitor = loadedMonitors[monitorId]
        loadRegionEditor(monitor)
    ...
    addOnTabReopen('regionEditor', function () {
        ...
        drawMonitorListToSelector(regionEditorMonitorsList,null,null,null,theSelectedServer)
    ...
    // drawSubMenuItems('regionEditor',pageLayouts['Region Editor'])
})
