$(document).ready(function(e){
    var theEnclosure = $('#tab-videosTable')
    var peerConnectKeysList = theEnclosure.find('.peerConnectKeys_list')
    ...
    var sideLinkListBox = $('#side-menu-link-videosTable ul')
    ...
    async function getSnapshotFromVideoTimeFrame(monitorId,startDate,endDate){
        ...
        if(typeof frameUrlCache[frameUrlCacheId] === 'string'){
            ...
            const frame = (await shinobiServer.getTimelapseFrames(monitorId,startDate,endDate,1))[0]
    ...
    window.openvideosTable = function(monitorId, peerConnectKey){
        drawPeerConnectKeysToSelector(peerConnectKeysList,null,null)
        drawMonitorListToSelector(monitorsList,null,null,true,peerConnectKey)
        peerConnectKeysList.val(peerConnectKey)
        monitorsList.val(monitorId)
        drawvideosTableElements()
        openTab(`videosTable`,{})
    }
    ...
    monitorsList.change(debounce(resetTable, 300));
    peerConnectKeysList.change(debounce(resetTable, 300));
    objectTagSearchField.change(debounce(resetTable, 300));
    cloudVideoCheckSwitch.change(debounce(resetTable, 300));
    ...
    async function drawvideosTableElements(pageNumber, pageSize, usePreloadedData) {
        // Set default values if pageNumber and pageSize are not provided
        pageNumber = pageNumber || 1;
        pageSize = pageSize || 10;

        var peerConnectKey = peerConnectKeysList.val();
        var shinobiServer = loadedShinobiAPI[peerConnectKey];
        var monitorId = monitorsList.val();
        var dateRange = shinobiServer.getSelectedTime(dateSelector);
        ...
        if (!usePreloadedData) {
            var result = await shinobiServer.getDetailedVideos({
                monitorId,
                startDate,
                endDate,
                searchQuery,
        ...
        videosTableDrawArea.bootstrapTable({
            ...
            onPageChange: (newPageNumber, newPageSize) => {
                drawvideosTableElements(newPageNumber, newPageSize);
                setTimeout(() => {
            ...
            data: loadedVideosTable.map((file) => {
                ...
                var loadedMonitor = shinobiServer.loadedMonitors[file.mid];
                return {
                    image: `<div class="video-thumbnail" data-mid="${file.mid}" data-ke="${file.ke}" data-time="${file.time}" data-end="${file.end}" data-filename="${file.filename}" data-peerconnectkey="${peerConnectKey}">
                    ...
                    buttons: `
                    <div class="row-info btn-group" data-mid="${file.mid}" data-ke="${file.ke}" data-time="${file.time}" data-filename="${file.filename}" data-status="${file.status}" data-type="${file.type}" data-peerconnectkey="${peerConnectKey}">
                        <a class="btn btn-sm btn-default open-video" href="${href}" title="${lang.Play}"><i class="fa fa-play"></i></a>
                        ${isLocalVideo && shinobiServer.permissionCheck('video_delete',file.mid) ? `<a class="btn btn-sm btn-${file.archive === 1 ? `success status-archived` : `default`} archive-video" title="${lang.Archive}"><i class="fa fa-${file.archive === 1 ? `lock` : `unlock-alt`}"></i></a>` : ''}
                        <div class="dropdown d-inline-block">
                            ...
                            <ul class="dropdown-menu shadow-lg">
                                ${shinobiServer.buildDefaultVideoMenuItems(file)}
                            </ul>
                        </div>
                    </div>
                    `,
    ...
    function getSelectedRows(getLoadedRows){
        ...
        videosTableDrawArea.find('[name="btSelectItem"]:checked').each(function(n,checkbox){
            ...
            rowsSelected.push(getLoadedRows ? shinobiServer.loadedVideosInMemory[`${monitorId}${time}${type}`] : {
                mid: monitorId,
                ke: groupKey,
                time: time,
                filename: filename,
    ...
    .on('click','.preview-video',function(e){
        ...
        var video = shinobiServer.loadedVideosInMemory[`${monitorId}${videoTime}${type}`]
        ...
        switch(data.f){
            case'fileBin_item_added':
                new PNotify({
                    ...
                    text: `${lang.checkFileBinForNewFile}<br><br><a class="btn btn-sm btn-success" href="${shinobiServer.buildFileBinUrl(data)}" download>${lang.Download}</a>`,
                    type: 'success',
                    sticky: true,
                })
            break;
            ...
            case'video_delete_cloud':
                if(tabTree.name === 'videosTable' && peerConnectKey === peerConnectKeysList.val()){
    ...
    addOnTabOpen('videosTable', function () {
        const firstServer = drawPeerConnectKeysToSelector(peerConnectKeysList,null,null)
        drawMonitorListToSelector(monitorsList,null,null,true,firstServer)
        drawvideosTableElements()
    })
    addOnTabReopen('videosTable', function () {
        ...
        theSelected = drawMonitorListToSelector(monitorsList,!theSelected,null,true,theSelectedServer)
        peerConnectKeysList.val(theSelectedServer)
        monitorsList.val(theSelected)
    })
    addOnTabAway('videosTable', function () {
